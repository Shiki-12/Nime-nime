...
export default async function PopularPage({ searchParams }: PopularPageProps) {
    ...
    try {
        ...
        fetchError = err instanceof Error ? err.message : "Failed to fetch popular anime.";
    }
            ...
            {animeList.length > 0 && (
                <>
                    ...
                    <div className="mt-10 flex items-center justify-center gap-2">
                        {pagination.hasPrev && (
                            <Link href={`/popular?page=${currentPage - 1}`} className="rounded-full bg-hn-card px-5 py-2 text-sm font-semibold text-white hover:bg-hn-card-hover">
                                ← Previous
                            </Link>
                        )}
